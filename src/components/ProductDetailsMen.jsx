...
function ProductDetailsMen() {
  const params = useParams();
  const context = useContext(Context);
  const filteredMenProducts = context.data.filter((item) =>{
    ...
    <div>
      <div className='flex items-center px-64 py-16'>
        <div className='flex items-center'>
          <div className=' '>
            <img className='py-2 h-32 w-96' src={filteredMenProducts[0].img} alt="" />
            <img className='py-2 h-32 w-full' src={filteredMenProducts[0].img} alt="" />
            <img className='py-2 h-32 w-full' src={filteredMenProducts[0].img} alt="" />
          </div>
          <img className='h-96 w-fit py-2 px-2' src={filteredMenProducts[0].img} alt="" />
        </div>
        <div>
          <h1 className='text-4xl font-bold px-8'>{filteredMenProducts[0].name}</h1>
          ...
          <div className='flex items-center px-8'>
            <p className='text-xl font-black text-green-500'>{filteredMenProducts[0].newPrice}$</p>
            <p className='ml-16 line-through text-lg opacity-45'>{filteredMenProducts[0].oldPrice}$</p>
          </div>
          <div className='text-sm mb-316 px-8 py-8'>
            {filteredMenProducts[0].description}
          </div>
          <div>
            ...
            <button onClick={()=>{addToCart(filteredMenProducts[0].id)}} className='px-6 py-2 ml-32 mt-2 border-2 border-black cursor-pointer rounded-xl shadow-2xl hover:bg-green-500 hover:text-white hover:scale-95'>Add to chart</button>
          </div>
        </div>
      </div>
    </div>
